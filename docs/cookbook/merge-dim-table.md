...
- One Iceberg dimension table you can join against from fact tables in other pipelines.
...
- Run the pipeline twice in a row. The **Volume timeline** on TableDetail shows two snapshots (one per run); the first is `append +N` (initial create), the second is `overwrite +N/-N` (the MERGE rewrote all rows via copy-on-write). Row count stays at N either way.
- Change a single row in the source (re-run `python scripts/synth_events.py --shape dim ... --revision 1` to bump customer #1's tier one band), re-run the pipeline. A third snapshot lands as `overwrite +N/-N`; the dim table has the same row count but customer #1's tier reflects the new value.
...
`UPDATE SET *` updates every non-key column from the source row. `INSERT *` inserts the whole source row. Both are Iceberg copy-on-write operations: each run produces a new snapshot, so time-travel queries (`SELECT ... FOR VERSION AS OF <snapshot_id>`) let you see the dimension as it was on any past run.
...
1. **Fix the transform, re-run.** Because the output is keyed, the next run upserts the corrected rows in place — no manual cleanup. The corrupted snapshot stays in Iceberg history for forensics; new queries see the fixed data.
...
**The dimension reflects partial data.** A run failed mid-way and only some keys got updated. Iceberg writes are atomic per snapshot — either every row in the run committed or none did. If you see partial updates, something else is writing to the table (a second pipeline targeting the same name? a stale CLI session?). The lineage panel on TableDetail surfaces the producing node.
